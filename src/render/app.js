...
import { PHYSICS, applyGravity, applyJump, applyHorizontalMovement, checkGroundCollision } from '../engine/physics.js';
...
const hudHP = document.createElement('div'); hudHP.style.fontWeight = '600';
const hudScore = document.createElement('div');
const hudGold = document.createElement('div');
const hudInv = document.createElement('div'); hudInv.style.color = '#61dafb'; hudInv.style.marginTop = '6px';
const hudFeedback = document.createElement('div'); hudFeedback.style.color = '#ffeb3b'; hudFeedback.style.marginTop = '6px';
...
(function loadShop() {
  if (typeof fetch !== 'undefined') {
    fetch('/assets/ui/shop.json').then(r => r.json()).then(j => {
      if (Array.isArray(j)) SHOP_ITEMS = j;
      else if (j && j.shop) SHOP_ITEMS = j.shop;
    }).catch(()=>{});
  }
})();
...
function openSettings() {
  if (!audio && typeof createAudioManager === 'function') audio = createAudioManager();
  ...
  const title = document.createElement('h2'); title.innerText = 'Settings'; box.appendChild(title);

  // Music volume
  const musicLabel = document.createElement('div'); musicLabel.innerText = 'Music Volume';
  const musicInput = document.createElement('input'); musicInput.type = 'range'; musicInput.min = 0; musicInput.max = 1; musicInput.step = 0.01;
  musicInput.value = (audio && audio.getMusicVolume) ? audio.getMusicVolume() : 0.6;
  musicInput.addEventListener('input', (e) => { if (audio && audio.setMusicVolume) audio.setMusicVolume(e.target.value); });
  box.appendChild(musicLabel); box.appendChild(musicInput);

  // SFX volume
  const sfxLabel = document.createElement('div'); sfxLabel.innerText = 'SFX Volume';
  const sfxInput = document.createElement('input'); sfxInput.type = 'range'; sfxInput.min = 0; sfxInput.max = 1; sfxInput.step = 0.01;
  sfxInput.value = (audio && audio.getSFXVolume) ? audio.getSFXVolume() : 1.0;
  sfxInput.addEventListener('input', (e) => { if (audio && audio.setSFXVolume) audio.setSFXVolume(e.target.value); });
  box.appendChild(sfxLabel); box.appendChild(sfxInput);

  // Render scale
  const scaleLabel = document.createElement('div'); scaleLabel.innerText = 'Render Scale (0.25 - 4.0)';
  const scaleInput = document.createElement('input'); scaleInput.type = 'range'; scaleInput.min = 0.25; scaleInput.max = 2; scaleInput.step = 0.01; scaleInput.value = 1;
  scaleInput.addEventListener('input', (e) => { if (renderer && renderer.setScale) renderer.setScale(e.target.value); });
  box.appendChild(scaleLabel); box.appendChild(scaleInput);
...
function createSettingsMenu() {
  ...
  musicInput.type = 'range'; musicInput.min = 0; musicInput.max = 1; musicInput.step = 0.01; musicInput.value = (audio && audio.getMusicVolume) ? String(audio.getMusicVolume()) : '0.6';
  ...
  sfxInput.type = 'range'; sfxInput.min = 0; sfxInput.max = 1; sfxInput.step = 0.01; sfxInput.value = (audio && audio.getSFXVolume) ? String(audio.getSFXVolume()) : '1.0';
  ...
  try { if (renderer && renderer.getScale) { const cur = String(renderer.getScale()); Array.from(scaleSelect.options).forEach(opt => { if (opt.value === cur) opt.selected = true; }); } } catch(e){}
  ...
  saveBtn.addEventListener('click', () => {
    ...
    try {
      ...
      if (audio && typeof audio.setMusicVolume === 'function') audio.setMusicVolume(mv);
      if (audio && typeof audio.setSFXVolume === 'function') audio.setSFXVolume(sv);
    } catch (e) { console.warn('Failed to apply audio settings', e); }

    // Apply renderer scale
    try { const s = parseFloat(scaleSelect.value); if (renderer && typeof renderer.setScale === 'function') renderer.setScale(s); } catch(e){}
    ...
    const info = document.createElement('div'); info.style.color='#9ccc65'; info.innerText='Settings saved'; box.appendChild(info);
...
const mainMenu = createMenu('main-menu-overlay', 'Main Menu', mainMenuItems, (i) => {
  if (i === 0) {
    ...
    if (!audio && typeof createAudioManager === 'function') audio = createAudioManager();
    if (audio) audio.playMusic('music_theme');
  } else if (i === 1) {
...
document.addEventListener('keydown', (e) => {
  if (menuVisible) {
    if (e.key === 'ArrowDown') { mainSelected = (mainSelected + 1) % mainMenu.list.children.length; setSelected(mainMenu.list, mainSelected); }
    if (e.key === 'ArrowUp') { mainSelected = (mainSelected - 1 + mainMenu.list.children.length) % mainMenu.list.children.length; setSelected(mainMenu.list, mainSelected); }
    if (e.key === 'Enter') { mainMenu.list.children[mainSelected].click(); }
    return;
  }

  if (pauseVisible) {
    if (e.key === 'ArrowDown') { pauseSelected = (pauseSelected + 1) % pauseMenu.list.children.length; setSelected(pauseMenu.list, pauseSelected); }
    if (e.key === 'ArrowUp') { pauseSelected = (pauseSelected - 1 + pauseMenu.list.children.length) % pauseMenu.list.children.length; setSelected(pauseMenu.list, pauseSelected); }
    if (e.key === 'Enter') { pauseMenu.list.children[pauseSelected].click(); }
    if (e.key === 'p') { pauseVisible = false; pauseMenu.overlay.style.display = 'none'; }
    return;
  }
  ...
  if (!menuVisible && !pauseVisible) {
    if (e.key === 'b') {
      ...
      if (player.gold < price) { player.feedback = 'Not enough gold'; player.feedbackExpire = performance.now() + FEEDBACK_LONG_MS; }
      else if (player.inventory.isFull()) { player.feedback = 'Inventory full'; player.feedbackExpire = performance.now() + FEEDBACK_LONG_MS; }
      else { player.inventory.addItem('Medkit', 1); player.gold -= price; player.feedback = `Bought Medkit for ${price}g`; player.feedbackExpire = performance.now() + FEEDBACK_LONG_MS; }
    }
    if (e.key === 'n') {
      const price = AMMO_PRICE; // ammo
      if (player.gold < price) { player.feedback = 'Not enough gold'; player.feedbackExpire = performance.now() + FEEDBACK_LONG_MS; }
      else if (player.inventory.isFull()) { player.feedback = 'Inventory full'; player.feedbackExpire = performance.now() + FEEDBACK_LONG_MS; }
      else { player.inventory.addItem('Ammo', 1); player.gold -= price; player.feedback = `Bought Ammo for ${price}g`; player.feedbackExpire = performance.now() + FEEDBACK_LONG_MS; }
    }
    if (e.key === 's' || e.key === 'S') {
      const items = player.inventory.getItems();
      if (items.length === 0) { player.feedback = 'Inventory empty'; player.feedbackExpire = performance.now() + FEEDBACK_LONG_MS; }
      else {
        const it = items[0];
...
function update(dt) {
  if (menuVisible || pauseVisible) return; // pause game updates while menus are visible
...
function frame(now) {
  ...
  if (renderer.drawGlow) renderer.drawGlow(player.x, player.y, player.width, player.height, player.color, 0.28);
  ...
  if (!menuVisible && !pauseVisible && Math.random() < 0.18) emitParticle(player.x + player.width/2, player.y + player.height/2);
  ...
  if (renderer.drawVignette) renderer.drawVignette();
