...
try {
  ...
  if (Array.isArray(parsed)) SHOP_ITEMS = parsed;
  else if (parsed && parsed.shop) SHOP_ITEMS = parsed.shop;
} catch (e) {
  // Browser/webpack fallback
  if (typeof fetch !== 'undefined') {
    fetch('/assets/ui/shop.json').then(r => r.json()).then(j => { if (Array.isArray(j)) SHOP_ITEMS = j; else if (j && j.shop) SHOP_ITEMS = j.shop; }).catch(()=>{});
  }
}
...
function main(): void {
  ...
  function loadMenuDefs(): { mainMenu: string[]; pauseMenu: string[] } {
    try {
      const p = path.join(process.cwd(), 'assets', 'ui', 'menu.json');
