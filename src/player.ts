...
export class Player {
  ...
  constructor(x: number = 0, y: number = 0, width: number = 28, height: number = 28) {
    this.pos = { x, y };
    this.vel = { x: 0, y: 0 };
    this.width = width;
    this.height = height;
  ...
  update(dt: number, tilemap: {isSolidAt: (tileX: number, tileY: number) => boolean}) {
  ...
  _collidesAt(x: number, y: number, tilemap: {isSolidAt: (tileX: number, tileY: number) => boolean}) {
